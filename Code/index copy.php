...
function closeNav() {
  ...
  document.getElementById("main").style.marginLeft= "0";
    ...
    <header class="pt2 pb1 align--center">
        ...
        <p style ="text-align: center; margin-left: 60px; margin-right: 60px;">Este projeto foi desenvolvido com o principal objetivo da definição de um esquema XML para Pontas de seta
    ...
    <div class="pt2 pb1 align--center">
...
// Define the XQuery string
$xquery = 'for $item in //arrowheads/arrowhead/raw_material/typometry[lenght=22]
           return <item>{ $item/name }</item>';

// Execute the XQuery in the eXist database
$query = $conexion->query($xquery);

// Fetch the results as an XML document
$xmlDoc = new DOMDocument();
$xmlDoc->loadXML($query->getRawResponse());

// Save the XML document as a file in the eXist database
$collection = '/db/apps/MegaLOD/arrowheads';
$filename = 'items.xml';
$resource = $connection->createResource($filename, $collection, $xmlDoc->saveXML());
$resource->setContent($xmlDoc->saveXML());
$resource->store();

// Output a message indicating the file was saved
echo 'The XML file was saved as ' . $collection . $filename;
?>
